...
auto-increment-offset = 1" > /etc/mysql/mysql.conf.d/mysqld.cnf
...
sudo mysql -e "CHANGE MASTER TO MASTER_HOST='10.0.1.100', MASTER_USER='repli', MASTER_PASSWORD='Caradhras2022'"

sudo mysql -e "start slave"

sudo wget http://owncloud.srs.epita.fr/s/7gf5IzADXJHkzUx/download
sudo mv download 2019.csv
sudo cp 2019.csv /var/lib/mysql-files/2019.csv

sudo mysql -e "USE nlpf; CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY NOT NULL, logins VARCHAR(100), email_address VARCHAR(200), surface NUMERIC, pieces NUMERIC, ville VARCHAR(200), types VARCHAR(100), budget NUMERIC, dates VARCHAR(100), code_postal VARCHAR(20), resultat NUMERIC, type_research VARCHAR(100))"

sudo mysql -e "USE nlpf; LOAD DATA INFILE '/var/lib/mysql-files/2019.csv' INTO TABLE immobilier FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 ROWS"
sudo touch isPassed.txt